...
- [ ] **Phase 범위 확인**: 요청 기능이 MVP(Phase 1) 범위인가, Phase 2 이상인가?
  ...
  - MVP 범위 외 목록: WYSIWYG 에디터, 블록 직접 추가/삭제, 커스텀 도메인, 방문자 분석 대시보드, 디자인 토큰 세부 편집, dnd-kit 드래그앤드롭
...
Phase 2는 사용자 개입 권한(커스터마이징)을 대폭 위임하는 단계입니다. 다음 **Step 단위**로 선형적(Linear) 구현을 진행해야 합니다.
...
|    **Step 2**     | **고급 CSS 커스텀**   | 고급 CSS 편집 기능 추가                                                     | `components/AdvancedCSSEditor.tsx`                           | **[완료]** |
