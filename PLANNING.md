...
> ⚠️ **Free 플랜 포트폴리오 수 제한 강제 시점**  
> `POST /api/portfolios` 호출 시점에 기존 포트폴리오 수를 DB에서 조회합니다.  
> 이미 1개 이상 존재하면 `403 plan_limit_exceeded` 를 반환하고, 클라이언트는 Pro 업그레이드 안내 모달을 표시합니다.  
> 대시보드에서도 Free 플랜 사용자가 포트폴리오를 1개 이미 보유한 경우 "새 포트폴리오 만들기" 버튼을 비활성화하고 업그레이드 유도 문구를 함께 노출합니다.
...
3. Pro 플랜 구독 → `seongjin.dev` 커스텀 도메인 연결 (Phase 2)
