...
export default function AdjustStep({ initialData }: { initialData?: {
  ...
  const SettingsPanel = (
    <div className="space-y-6">
      ...
      {/* Domain Settings - Spotify Premium Dark Redesign */}
      <div className="bg-[#121212] border border-white/5 rounded-[32px] p-6 sm:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.2)] text-white space-y-6">
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-black uppercase text-spotify-silver tracking-wider">기본 무료 제공 주소</Label>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-white/5 border border-white/5 rounded-2xl">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[15px] font-bold text-white font-mono truncate max-w-[200px] sm:max-w-xs">{initialData?.slug}.portfolioforge.app</span>
              </div>
          ...
          <div className="space-y-2.5 pt-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="custom-domain" className="text-xs font-black uppercase text-spotify-silver tracking-wider">
                나만의 커스텀 도메인 연결 <span className="text-[10px] text-spotify-silver/50 lowercase font-medium">(optional)</span>
              </Label>
            </div>
          ...
          <div className="border border-white/5 rounded-2xl overflow-hidden bg-white/[0.02]">
            ...
            >
              <span className="flex items-center gap-2">
                <Settings className="w-4 h-4 text-spotify-green animate-spin" style={{ animationDuration: '6s' }} />
                DNS 레코드 수동 설정 가이드
              </span>
              {showDnsManual ? <ChevronUp className="w-4 h-4 text-spotify-silver" /> : <ChevronDown className="w-4 h-4 text-spotify-silver" />}
            </button>
              ...
              <div className="p-4 border-t border-white/5 space-y-4 text-[12px] text-spotify-silver leading-relaxed animate-in slide-in-from-top-2 duration-300">
                ...
                <div className="space-y-2 font-mono text-[11px] text-white">
                  <div className="p-3 bg-spotify-near-black border border-white/5 rounded-xl flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-spotify-silver uppercase block font-sans">Type A (루트 도메인용)</span>
                      <span>Name: <strong className="text-spotify-green">@</strong> | Value: <strong>76.76.21.21</strong></span>
                    </div>
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <div className="p-3 bg-spotify-near-black border border-white/5 rounded-xl flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-spotify-silver uppercase block font-sans">Type CNAME (서브 도메인용)</span>
                      <span>Name: <strong className="text-spotify-green">www</strong> | Value: <strong>cname.vercel-dns.com</strong></span>
                    </div>
                    <Button
                      variant="ghost"
                      size="sm"
          ...
          {customDomain && (
            <div className="p-5 bg-spotify-green/5 border border-spotify-green/20 rounded-2xl space-y-3 animate-in fade-in duration-300">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (data.configured) {
                      toast.success("도메인 연결이 성공적으로 시뮬레이션 및 연결 완료되었습니다!");
                    } else {
              ...
              <div className="text-[12px] text-spotify-silver font-medium leading-relaxed">
                현재 <strong className="text-white font-mono">{customDomain}</strong> 주소가 포트폴리오에 등록되어 있습니다. 
                DNS 전파는 최대 24~48시간이 소요될 수 있으며, 모의 우회 설정에 따라 정상 연결로 확인됩니다.
              </div>
            </div>
          )}
        </div>
