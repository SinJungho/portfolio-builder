...
#### 💻 실무 표준 지향적 명명법 및 구조적 리팩토링 규칙 (Professional Naming & Structural Refactoring)
* **명확한 정식 명칭 사용**: 모호하거나 극도로 생략된 임의 축약형 변수명/매개변수명(예: `val`, `prev`) 대신 문맥적으로 정확하고 가독성이 뛰어난 실무 표준 정식 명칭(예: `value`, `prevIds`, `prevDescriptions`, `domainValue`)을 사용합니다.
* **이벤트 핸들러 및 비즈니스 로직 격리**: JSX 마크업 내부 인라인에 무거운 연산이나 API 통신, 비동기 호출, 클립보드 복사 등의 로직을 직접 선언하여 작성하지 않습니다. 모든 비즈니스 로직은 컴포넌트 본문 최상단에 독립적인 **이벤트 핸들러 함수군**(`handleConnectDomain`, `handleCheckStatus` 등)으로 격리 정의하여 사용합니다.
* **서브 컴포넌트 적극적 분리**: 비대하거나 동일한 레이아웃 구조가 중복적으로 사용되는 마크업(예: 레코드 상세 가이드 등)은 독립적이고 재사용 가능한 **하위/서브 컴포넌트**(`DNSRecordItem` 등)로 적극 분리하여 코드베이스의 중복을 제거하고 가독성을 높입니다.
* **실무급 비동기 사용자 경험(UX) 확보**: 모든 비동기 통신 및 상태 조회 처리 시 중복 전송 방지를 막는 로딩 스테이트(`isChecking`, `isLoading`)와 버튼 비활성화(`disabled`), 시각적인 동적 로딩 스피너 기법(`animate-spin`)을 완벽하게 장착하여 실무 프로덕션 제품의 수준에 부합하도록 설계합니다.
...
#### 🚨 린트 경고 및 에러 강제 억제 금지 규칙 (No Inline Lint Disabling)
* 소스코드 내부에서 `// eslint-disable-next-line`, `/* eslint-disable */` 등의 주석을 사용하여 린트 에러나 경고를 강제로 끄거나 무시하게끔 우회하는 행위는 **절대적으로 금지**합니다.
* 모든 린트 피드백과 경고는 실제 코드의 논리적 문제, 타입 불일치, 혹은 안티패턴을 가리키는 지표이므로, 주석 소멸 우회가 아닌 **명확한 타입 명시 및 올바른 React 아키텍처로 코드를 직접 개선하여 정상 해결**해야 합니다.

#### 🌐 Vercel 배포 빌드 규격 일치 (Production Build & Lint Parity)
* 로컬 개발 단계와 Vercel 배포 단계의 품질 규격을 완벽하게 일치시킵니다.
* 배포 환경인 Vercel 빌드 타임에서는 TypeScript 컴파일 및 ESLint 정적 분석 과정에서 경고가 단 하나라도 발생할 경우 배포 빌드가 실패(Build Fail)하도록 엄격히 조성합니다.
* 로컬의 `pnpm lint` 및 `pnpm run build`는 Vercel 배포 사전 검증 환경과 100% 동일하게 동작하므로, 어떠한 오류나 경고도 허용되지 않는 상태(Zero-Warning State)를 달성한 후에만 프로덕션 배포 및 최종 커밋을 진행합니다.
